...
static guiWindowData *getBackendData()
{
    return ImGui::GetCurrentContext()
               ? (guiWindowData *)ImGui::GetIO().BackendPlatformUserData
               : nullptr;
}
...
{
    guiWindowData *bd = getBackendData();
...
static void setPlatformImeData_SDL(ImGuiViewport *,
...
guiWindow::guiWindow(std::shared_ptr<Window> window) : m_windowHandle(window)
{
    ImGuiIO &io = ImGui::GetIO();
    IM_ASSERT(io.BackendPlatformUserData == nullptr &&
              "Already initialized a platform backend!");

    guiWindowData *bd = new guiWindowData;
    ...
    if (std::dynamic_pointer_cast<SDLWindow>(window) != nullptr)
    {
        m_windowBackendType = WINDOW_BACKEND_TYPE_SDL;
        m_inputHandle = std::make_shared<SDLInputSystem>();
        ...
        if (SDL_GetWindowWMInfo(
                static_cast<SDL_Window *>(m_windowHandle->getWindowHandle()),
                &info))
        {
    ...
    // m_rendererHandle->m_surface =
    // m_windowHandle->createSurface(m_rendererHandle->m_renderContextHandle->getInstanceHandle());

    Engine::getMessageHandler()->addWindowMouseFocusOnCallback([](void *)
        { static_cast<guiWindowData *>(ImGui::GetIO().BackendPlatformUserData)->PendingMouseLeaveFrame = 0; });
    Engine::getMessageHandler()->addWindowMouseFocusOffCallback([](void *)
        { static_cast<guiWindowData *>(ImGui::GetIO().BackendPlatformUserData)->PendingMouseLeaveFrame = ImGui::GetFrameCount() + 1; });
    Engine::getMessageHandler()->addWindowKeyboardFocusOnCallback(
        [](void *)
        { ImGui::GetIO().AddFocusEvent(true); });
    Engine::getMessageHandler()->addWindowKeyboardFocusOffCallback(
        [](void *)
        { ImGui::GetIO().AddFocusEvent(false); });
    Engine::getMessageHandler()->addMouseMotionCallback(
        [](int32_t x, int32_t y, bool touch)
        {
            ImGui::GetIO().AddMouseSourceEvent(touch ? ImGuiMouseSource_TouchScreen : ImGuiMouseSource_Mouse);
            ImGui::GetIO().AddMousePosEvent(x, y);
        });
    Engine::getMessageHandler()->addMouseWheelCallback(
        [](int32_t wheelX, int32_t wheelY, bool touch)
        {
    ...
    Engine::getMessageHandler()->addMouseButtonDownCallback(
        [&](eMouseKeyCode mouseKey, int32_t x, int32_t y, bool touch)
        {
            int mouseButton = -1;
            if (mouseKey == eMouseKeyCode::MOUSE_KEY_CODE_LB)
            {
                mouseButton = 0;
            }
            if (mouseKey == eMouseKeyCode::MOUSE_KEY_CODE_RB)
            {
                mouseButton = 1;
            }
            if (mouseKey == eMouseKeyCode::MOUSE_KEY_CODE_MB)
            {
                mouseButton = 2;
            }
            if (mouseKey == eMouseKeyCode::MOUSE_KEY_CODE_X1B)
            {
                mouseButton = 3;
            }
            if (mouseKey == eMouseKeyCode::MOUSE_KEY_CODE_X2B)
            {
                mouseButton = 4;
            }
            if (mouseButton == -1)
            ...
            static_cast<guiWindowData *>(ImGui::GetIO().BackendPlatformUserData)->MouseButtonsDown |= (1 << mouseButton);
        });
    Engine::getMessageHandler()->addMouseButtonUpCallback(
        [&](eMouseKeyCode mouseKey, int32_t x, int32_t y, bool touch)
        {
            int mouseButton = -1;
            if (mouseKey == eMouseKeyCode::MOUSE_KEY_CODE_LB)
    ...
    Engine::getMessageHandler()->addKeyDownCallback([&](eKeyCode key)
        { ImGui::GetIO().AddKeyEvent(keyCodeTranslatorBack(key), true); });
    Engine::getMessageHandler()->addKeyUpCallback(
        [&](eKeyCode key)
        { ImGui::GetIO().AddKeyEvent(keyCodeTranslatorBack(key), false); });
    Engine::getMessageHandler()->addTextInputCallback(
        [](const char *text)
...
void guiWindow::newFrame()
{
    ...
    if (m_windowHandle->isMinimized())
        size = {0u, 0U};
    auto displaySize = m_windowHandle->getDisplaySize();
    io.DisplaySize =
        ImVec2(static_cast<float>(size.first), static_cast<float>(size.second));
    if (io.DisplaySize.x > 0 && io.DisplaySize.y > 0)
        io.DisplayFramebufferScale =
            ImVec2(static_cast<float>(displaySize.first) / io.DisplaySize.x,
                   static_cast<float>(displaySize.second) / io.DisplaySize.y);
    ...
    static size_t frequency =
        (m_windowBackendType == WINDOW_BACKEND_TYPE_SDL)
            ? std::dynamic_pointer_cast<SDLWindow>(m_windowHandle)->getFrequency()
            : 1U;
    ...
    io.DeltaTime =
        bd->Time > 0
            ? static_cast<float>(static_cast<double>(current_time - bd->Time) /
                                 frequency)
...
void guiWindow::updateMouseData()
{
    guiWindowData *bd = getBackendData();
    ImGuiIO &io = ImGui::GetIO();
    ...
    if (m_windowHandle->isFocused())
    {
        ...
        if (io.WantSetMousePos)
            m_inputHandle->setCursorPos({io.MousePos.x, io.MousePos.y},
        ...
        {
            auto cursorPos = m_inputHandle->getCursorPos(
                eCursorCoordinate::CURSOR_COORDINATE_SCREEN);
            auto pos = m_windowHandle->getPosition();
            io.AddMousePosEvent(static_cast<float>(cursorPos.first - pos.first),
                                static_cast<float>(cursorPos.second - pos.second));
        }
    }
}

void guiWindow::updateMouseCursor()
{
    ImGuiIO &io = ImGui::GetIO();
...
eMouseCursor guiWindow::mouseCursorTranslator(ImGuiMouseCursor cursor)
{
    switch (cursor)
    {
...
ImGuiKey guiWindow::keyCodeTranslatorBack(eKeyCode keyCode)
{
    switch (keyCode)
    {
